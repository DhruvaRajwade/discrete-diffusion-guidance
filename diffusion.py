...
class Diffusion(L.LightningModule):
        ...
        if config.parameterization == "ar":
            ...
            if self.diffusion == "absorbing_state":
                ...
                limiting_distribution = (
                    torch.ones((1, 1, self.vocab_size), dtype=self.dtype)
                    / self.vocab_size
                )
            else:
            ...
            self.register_buffer("limiting_distribution", limiting_distribution)
    ...
    def on_load_checkpoint(self, checkpoint):
        if self.limiting_distribution is not None:
            checkpoint["state_dict"]["limiting_distribution"] = (
                self.limiting_distribution.to(
    ...
    def on_save_checkpoint(self, checkpoint):
        ...
        checkpoint["state_dict"].pop("limiting_distribution", None)
        if self.ema:
    ...
    def _d3pm_loss(self, model_output, xt, x0, t):
        ...
        if self.diffusion == "absorbing_state":
            log_x_theta_at_x0 = torch.gather(model_output, -1, x0[:, :, None]).squeeze(
                -1
            )
    ...
    def _reconstruction_loss(self, x0, cond=None):
        ...
        assert self.config.noise.type == "loglinear", (
            "Reconstruction loss only implemented for log-linear noise."
        )
        t0 = torch.zeros(x0.shape[0], dtype=self.dtype, device=self.device)
        time_conditioning = self.noise(t0)[0][:, None]
        model_output_t0 = self.forward(x0, time_conditioning, cond=cond)
        return -torch.gather(
    ...
    def _q_xt(self, x, move_chance):
        ...
        if self.diffusion == "uniform":
            ...
            return torch.where(move_indices, self._sample_prior(*x.shape), x)
        raise NotImplementedError(f"Diffusion type {self.diffusion} not implemented.")
    ...
    def _forward_pass_diffusion(self, x0, cond=None):
        ...
        model_output = self.forward(xt, time_conditioning, cond=cond)
        ...
        if self.T > 0:
            ...
            if self.parameterization == "d3pm":
                reconstruction_loss = self._reconstruction_loss(x0, cond=cond)
        ...
        if self.diffusion == "absorbing_state":
            ...
            if self.change_of_variables or self.importance_sampling:
                if self.training and self.config.training.use_simple_ce_loss:
                    return {
                        "diffusion_loss": log_p_theta
                        * torch.log1p(-torch.exp(-self.noise.sigma_min)),
                        "loss": -log_p_theta,
                    }
            ...
            #    alpha_t = (-sigma).exp()
            alpha_t_prime = -1.0
            alpha_t = 1.0 - t[..., None, None]  # B, 1, 1

            # x_bar = N * α_t * x + 1 - α_t ; B, L, V
            x_bar = (
                self.vocab_size * alpha_t * F.one_hot(x0, self.vocab_size).float()
                + 1
                - alpha_t
            )
            ...
            term1 = (self.vocab_size / x_bar_zt) - (
                self.vocab_size / x_bar_theta_zt
            )  # B, L, 1
            ...
            return {
                ...
                "diffusion_loss": diffusion_loss,
                "loss": diffusion_loss
                if getattr(self.config, "zero_recon_loss", False)
    ...
    def _loss(self, x0, attention_mask, cond=None):
        ...
        if (
            ...
            # guidance training.
            p = torch.bernoulli(
                torch.ones_like(cond) * self.config.training.guidance.cond_dropout
            ).to(torch.bool)
            # Use num_classes index as conditioning mask_token_id
            cond[p] = self.config.data.num_classes

        if self.parameterization == "ar":
            logprobs = self.forward(input_tokens, sigma=None, cond=cond)
        ...
        if self.config.training.compute_loss_on_pad_tokens and self.training:
            token_nll = loss.mean()
        else:
            batch_nll = nlls.sum()
    ...
    def _compute_loss(self, batch, prefix):
        ...
        cond = None
        if (
            self.config.training.guidance is not None  # Training for / using CFG
            or (
            ...
            if self.config.data.label_col in batch:
                ...
                raise RuntimeError(
                    f"Conditioning {self.config.data.label_col} not found in batch."
                )
        ...
        if (
            ...
            if self.config.is_vision:
                ...
                if self.config.training.guidance is not None:
                    ...
                    guidance = {"method": "cfg", "condition": 0, "gamma": 1.0}
                    omegaconf.OmegaConf.update(
                        self.config, key="guidance", value=guidance, force_add=True
                    )
                ...
                if self.config.training.guidance is not None:
                    ...
                    for i in range(self.config.data.num_classes):
                        self.config.guidance.condition = i
                        samples = self.sample()
                        decoded_samples = self.tokenizer.batch_decode(samples)
    ...
    def _sample_prior(self, *batch_dims):
        ...
        if self.diffusion == "uniform":
            ...
            return torch.distributions.Categorical(
                self.limiting_distribution.squeeze()
            ).sample(sample_shape=torch.Size(batch_dims))
        raise NotImplementedError(f"Diffusion type {self.diffusion} not implemented.")

    def sample(self, eps=1e-5):  # Note: differs from self.config.training.sampling_eps
    ...
    def _ar_sample(
        ...
        inference_params = InferenceParams(
            max_seqlen=num_pred_tokens, max_batch_size=x.shape[0], seqlen_offset=1
        )
        ...
        for i in pbar:
            if getattr(self.config, "guidance", None) is None:
                if self.config.backbone == "dimamba":
                    ...
                    log_probs = self.forward(x[:, : i + 1], None, cond=None)
                if self.config.sampling.use_float64:
                ...
                if self.config.guidance.method == "cfg":
                    if self.config.backbone == "dimamba":
                        next_log_probs = self._ar_cfg_denoise(
                            ...
                            inference_params=(
                                inference_params,
                                uncond_inference_params,
                            ),
                        )
                    else:
                        next_log_probs = self._ar_cfg_denoise(
                            cond=cond, gamma=self.config.guidance.gamma, x=x, i=i
                        )
                    y = (next_log_probs + noise[:, i]).argmax(-1)
    ...
    def _ar_cfg_denoise(
        self, cond: torch.tensor, gamma: float, x: torch.tensor, i: int, **kwargs
    ) -> torch.tensor:
        if self.config.guidance.gamma == 0.0:  # Sample unconditionally
            ...
            if self.config.backbone == "dimamba":
                ...
                log_probs = self.forward(x[:, : i + 1], None, cond=mask_cond, **kwargs)
        elif gamma == 1.0:  # Sample conditionally
            if self.config.backbone == "dimamba":
                inference_params = kwargs.pop("inference_params")
                log_probs = self.forward(
                    x[:, : i + 1], None, cond=cond, inference_params=inference_params[0]
                )
            else:
                log_probs = self.forward(x[:, : i + 1], None, cond=cond, **kwargs)
            ...
            if self.config.backbone == "dimamba":
                ...
                log_probs_cond = self.forward(x[:, : i + 1], None, cond=cond, **kwargs)
                log_probs_uncond = self.forward(
    ...
    def _ar_fudge_denoise(
        ...
        **kwargs,
        ...
        log_probs = self.forward(x[:, : i + 1], None, cond=None, **kwargs)
        next_log_probs = log_probs[:, -1]
        top_logits, top_indices = next_log_probs.topk(topk, dim=-1)
        t_candidates = torch.cat(
            [x[:, : i + 1].unsqueeze(1).expand(-1, topk, -1), top_indices.unsqueeze(2)],
            dim=2,
        ).view(-1, i + 2)  # (B * K), L

        t = torch.zeros(t_candidates.shape[0], device=self.device)
        sigma, dsigma = self.noise(t)
        time_conditioning = sigma[:, None]
    ...
    def _diffusion_sample(
        ...
        xt = self._sample_prior(
            self.config.sampling.batch_size, self.config.model.length
        ).to(self.device)
    ...
    def _ddpm_denoise(
        ...
        if self.diffusion == "absorbing_state":
            ...
            q_xs = self._compute_posterior(
                x=x_theta, xt=xt, alpha_s=1 - move_chance_s, alpha_t=1 - move_chance_t
            )
        else:
            raise NotImplementedError(
                f"Diffusion type {self.diffusion} not implemented."
    ...
    def _cfg_denoise(
        ...
        if cache is not None:
            ...
            if gamma == 0.0:  # Sample unconditionally
                mask_cond = torch.ones_like(cond) * self.config.data.num_classes
                log_x_theta_uncond = self.forward(xt, time_conditioning, cond=mask_cond)
                log_x_theta_cond = None
            elif gamma == 1.0:  # Sample conditionally
                log_x_theta_cond = self.forward(xt, time_conditioning, cond=cond)
        ...
        if (
            log_x_theta_cond is None  # gamma == 0
            or log_x_theta_uncond is None
        ):  # or gamma == 1
            log_x_theta = (
                log_x_theta_uncond
                if log_x_theta_uncond is not None
                else log_x_theta_cond
            )
            x_theta = log_x_theta.exp()
            if self.diffusion == "absorbing_state":
                q_xs = x_theta * (move_chance_t - move_chance_s)
            ...
            if self.diffusion == "absorbing_state":
                log_x_theta = (
                    gamma * log_x_theta_cond + (1 - gamma) * log_x_theta_uncond
                )
                ...
                log_q_xs = gamma * log_q_xs_cond + (1 - gamma) * log_q_xs_uncond
        ...
        return (
            ...
            q_xs,
            {
                "log_x_theta_uncond": log_x_theta_uncond,
    ...
    def _cbg_denoise(
        ...
        if cache is not None:
            ...
            # Classifier model
            if use_approx:
                xt_one_hot = torch.nn.functional.one_hot(xt, self.vocab_size).to(
                    torch.float
                )
                ...
                classifier_log_prob_ratio = (
                    (
                        grad_log_prob_xt
                        - (xt_one_hot * grad_log_prob_xt).sum(dim=-1, keepdim=True)
                    )
                    .detach()
                ...
                classifier_log_prob = (
                    (
                        classifier_log_prob_ratio
                        + classifier_log_prob_xt[..., conditioning_class][
                            ..., None, None
                        ]
                    )
                    .detach()
        ...
        if self.diffusion == "absorbing_state":
            diffusion_log_probs = log_x_theta + torch.log(
                1.0 - (move_chance_s / move_chance_t)
            )
            diffusion_log_probs[..., self.mask_index] = torch.log(
                move_chance_s / move_chance_t
            )[:, :, 0]
        ...
        return (
            xs,
            guided_probs,
            {"log_x_theta": log_x_theta, "classifier_log_prob": classifier_log_prob},
        )
    ...
    def _nos_denoise(
        ...
        with torch.no_grad():
            ...
            with torch.cuda.amp.autocast(dtype=torch.float32):
                logits, hidden_states = self.backbone(
                    xt, time_conditioning, cond=None, return_hidden_states=True
                )
                if self.parameterization == "subs":
        ...
        delta = torch.nn.Parameter(
            torch.zeros_like(hidden_states[-1]), requires_grad=True
        )
        ...
        with torch.enable_grad():
            for _ in tqdm(range(num_nos_steps), desc="NOS", leave=False):
                ...
                target_loss = classifier_model.get_log_probs(
                    xt, time_conditioning, x_emb=h_current
                )[..., conditioning_class].sum()
                with torch.cuda.amp.autocast(dtype=torch.float32):
                    new_logits = self.forward(
                        xt, time_conditioning, cond=None, x_emb=h_current
                    )
                if self.diffusion == "absorbing_state":
                    adjusted_log_probs = new_logits + torch.log(
                        1.0 - (move_chance_s / move_chance_t)
                    )
                    adjusted_log_probs[..., self.mask_index] = torch.log(
                        move_chance_s / move_chance_t
                    )[:, :, 0]
        ...
        with torch.cuda.amp.autocast(dtype=torch.float32):
            guided_logits = self.forward(
                xt, time_conditioning, cond=None, x_emb=hidden_states[-1] + delta.data
            )
        if self.diffusion == "absorbing_state":
