...
class DummyVisionTokenizer:
    def __init__(
        self, vocab_size, image_size, add_mask_token=True, add_special_tokens=True
    ):
    ...
    def batch_decode(self, x):
        return einops.rearrange(x, "b (c h w) -> b c h w", c=3, h=self.image_size)

    def decode(self, x):
        return einops.rearrange(x, "(c h w) -> c h w", c=3, h=self.image_size)
...
class DiscreteCIFAR10(torchvision.datasets.CIFAR10):
    ...
    def __getitem__(self, index):
        ...
        return {"input_ids": img, "labels": target, "attention_mask": attention_mask}
