...
class Classifier(L.LightningModule):
    def __init__(
        ...
        if config.data.num_classes > 2:
            ...
            metrics.add_metrics(
                {"precision": Precision(class_idx=1), "recall": Recall(class_idx=1)}
            )
    ...
    def configure_optimizers(self):
        ...
        optimizer = torch.optim.AdamW(
            itertools.chain(
                self.classifier_model.parameters(), self.noise.parameters()
            ),
    ...
    def _q_xt(self, x, move_chance):
        ...
        raise NotImplementedError(
            f"Diffusion type {self.config.diffusion} not implemented."
        )
    ...
    def _compute_loss(self, batch, prefix):
        ...
        if hasattr(self.config.data, "label_col"):
            ...
            y = batch["label"]
        if not self.is_eval_classifier and getattr(
            self.config.training, "use_label_smoothing", False
        ):
            # Interpolate between one-hot and uniform distribution
            labels = (
                torch.nn.functional.one_hot(y, self.config.data.num_classes)
                * (1 - t)[..., None]
        ...
        if getattr(self.config, "is_fudge_classifier", False):
            ...
            logits = logits.view(-1, self.config.data.num_classes)[
                attention_mask.flatten() == 1, ...
            ]
            y = expanded_y.flatten().long()[attention_mask.flatten() == 1]
            loss = torch.nn.functional.cross_entropy(
                logits, y, ignore_index=-100, reduction="mean"
            )
        else:
            loss = torch.nn.functional.cross_entropy(
                logits.view(-1, logits.size(-1)),
