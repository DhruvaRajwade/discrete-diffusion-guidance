...
class DDiTBlock(nn.Module):
    ...
    def forward(self, x, rotary_cos_sin, c, seqlens=None):
        ...
        if self.use_adaLN:
            ...
            x = self.norm2(x)
        return bias_dropout_scale_fn(self.mlp(x), None, gate_mlp, x_skip, self.dropout)
...
class DDitFinalLayer(nn.Module):
    def __init__(self, hidden_size, out_channels, cond_dim, use_adaLN=True):
        super().__init__()
...
class DIT(nn.Module, huggingface_hub.PyTorchModelHubMixin):
    def __init__(self, config, vocab_size):
        ...
        blocks = []
        use_adaLN = config.parameterization != "ar" or self.cond_map is not None
    ...
    def forward(
        self, indices, sigma, cond=None, x_emb=None, return_hidden_states=False
    ):
    ...
    def forward(self, indices_or_one_hots, sigma, x_emb=None, attention_mask=None):
        if x_emb is None:
            if indices_or_one_hots.ndim == 2:  # indices (B, L)
                ...
                x = F.linear(
                    indices_or_one_hots.to(torch.float), self.vocab_embed.embedding.T
                )
