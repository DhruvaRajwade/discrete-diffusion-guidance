...
class Block(nn.Module):
    ...
    def forward(
        ...
        if self.use_adaLN and cond_embeds is not None:
            (shift_msa, scale_msa, gate_msa) = self.adaLN_modulation(cond_embeds)[
                :, None
            ].chunk(3, dim=-1)
        ...
        if self.use_adaLN and cond_embeds is not None:
            ...
            hidden_states = bias_dropout_scale_fn(
                hidden_states, None, gate_msa, residual, self.dropout
            )
...
class BiMambaWrapper(nn.Module):
    ...
    def forward(self, hidden_states, inference_params=None):
        ...
        if self.bidirectional:
            if inference_params is not None:
                raise NotImplementedError(
                    "Passing `inference_params` not supported for bidirectional Mamba."
                )
    ...
    def allocate_inference_cache(self, batch_size, max_seqlen, dtype=None, **kwargs):
        if self.bidirectional:
            raise NotImplementedError(
                "Allocating inference cache not supported for bidirectional Mamba."
            )
        ...
        if hidden_states is None:
            ...
            if not self.fused_add_norm:
                if self.config.use_adaLN:
                    raise NotImplementedError(
                        "adaln only implemented for fused_add_norm"
                    )
                residual = (
                ...
                if cond_embeds is not None and self.config.use_adaLN:
                    shift, scale = self.adaLN_modulation_final(cond_embeds)[
                        :, None
                    ].chunk(2, dim=2)
                else:
            ...
            if cond_embeds is not None and self.config.use_adaLN:
                shift, scale = self.adaLN_modulation_final(cond_embeds)[:, None].chunk(
                    2, dim=2
                )
                hidden_states = modulate_fused(hidden_states, shift, scale)
...
class DiMamba(nn.Module, huggingface_hub.PyTorchModelHubMixin):
    def __init__(self, config, vocab_size: int, pad_token_id: int):
        ...
        if (
            config.training.guidance is not None  # Training for / using CFG
            or (
        ...
        mamba_config = BiMambaConfig(
            ...
            bidirectional_strategy=getattr(
                config.model, "bidirectional_strategy", "add"
            ),
            bidirectional_weight_tie=getattr(
...
class DiMambaClassifier(nn.Module):
    def __init__(self, config, vocab_size: int, pad_token_id: int):
        ...
        mamba_config = BiMambaConfig(
            ...
            bidirectional_strategy=getattr(
                config.classifier_model, "bidirectional_strategy", "add"
            ),
            bidirectional_weight_tie=getattr(
                config.classifier_model, "bidirectional_weight_tie", True
            ),
        ...
        self.output_layer = nn.Linear(
            config.classifier_model.hidden_size, config.classifier_model.num_classes
        )
        ...
        with torch.cuda.amp.autocast(dtype=torch.bfloat16):
            if self.pooling == "mean":
                ...
                x = torch.sum(masked_x, dim=1) / (
                    torch.sum(attention_mask, dim=1, keepdim=True) + 1e-15
                )
            else:
                raise NotImplementedError(f"`{self.pooling}` method not implemented.")
