...
def main(config: omegaconf.DictConfig) -> None:
    ...
    pctile_threshold_value = np.percentile(qm9_dataset[label_col], q=pctile_threshold)
    ...
    result_dicts.append(
        {
            ...
            f"Novel {label_col.upper()} 25%ile": np.percentile(
                qm9_dataset[label_col], q=25
            ),
            f"Novel {label_col.upper()} Median": np.median(qm9_dataset[label_col]),
            f"Novel {label_col.upper()} 75%ile": np.percentile(
                qm9_dataset[label_col], q=75
            ),
        }
    ...
    result_dicts.append(
        {
            ...
            "Num Samples": config.sampling.batch_size
            * config.sampling.num_sample_batches,
            "Valid": valid_pct,
            "Unique": unique_pct,
            "Novel": novel_pct,
            f"{label_col.upper()} Mean": np.mean(mol_property)
            if len(mol_property) > 0
            else 0.0,
            f"{label_col.upper()} 25%ile": np.percentile(mol_property, q=25)
            if len(mol_property) > 0
            else 0.0,
            f"{label_col.upper()} Median": np.median(mol_property)
            if len(mol_property) > 0
            else 0.0,
            f"{label_col.upper()} 75%ile": np.percentile(mol_property, q=75)
            if len(mol_property) > 0
            else 0.0,
            f"Novel {label_col.upper()} Mean": np.mean(mol_property_novel)
            if len(mol_property_novel) > 0
            else 0.0,
            f"Novel {label_col.upper()} 25%ile": np.percentile(mol_property_novel, q=25)
            if len(mol_property_novel) > 0
            else 0.0,
            f"Novel {label_col.upper()} Median": np.median(mol_property_novel)
            if len(mol_property_novel) > 0
            else 0.0,
            f"Novel {label_col.upper()} 75%ile": np.percentile(mol_property_novel, q=75)
            if len(mol_property_novel) > 0
            else 0.0,
        }
    ...
    print(
        "Guidance:",
        ", ".join([f"{k.capitalize()} - {v}" for k, v in config.guidance.items()]),
    )
    print(
        f"\tValid: {valid:,d} / {len(samples):,d} ({100 * valid_pct:0.2f}%) ",
        f"Unique (of valid): {unique:,d} / {valid:,d} ({100 * unique_pct:0.2f}%) ",
        f"Novel (of valid): {novel:,d} / {valid:,d} ({100 * novel_pct:0.2f}%)\n",
        f"\t{label_col.upper()} Mean: {np.mean(mol_property) if len(mol_property) else 0.0:0.3f}, {label_col.upper()} Median: {np.median(mol_property) if len(mol_property) else 0.0:0.3f}\n",
        f"\tNovel {label_col.upper()} Mean: {np.mean(mol_property_novel) if len(mol_property_novel) else 0.0:0.3f}, Novel {label_col.upper()} Median: {np.median(mol_property_novel) if len(mol_property_novel) else 0.0:0.3f}",
    )
