...
def generate_ordered_kmers(kmer_length: int) -> typing.List[str]:
    """
    ...
    kmers = [
        "".join(kmer) for kmer in itertools.product(characters, repeat=kmer_length)
    ]
...
def compute_kmer_frequencies(
    ...
    for kmer in kmer_list:
        try:
            kmer_frequencies.append(kmer_counts[kmer] / count_kmers_occurrences)
        except KeyError:
...
def run_eval_pipeline(
    ...
    tokenizer = transformers.AutoTokenizer.from_pretrained(
        classifier_config.data.tokenizer_name_or_path, trust_remote_code=True
    )
    ...
    if os.path.exists(train_weights_path):
        ...
        train_weights = {
            k: v / np.sum(list(train_weights.values()))
    ...
    if os.path.exists(val_weights_path):
        ...
        val_weights = {
            k: v / np.sum(list(val_weights.values())) for k, v in val_weights.items()
        }
    ...
    test_preds = [
        pretrained_classifier.forward(
            test_dataset[i]["input_ids"][None, ...].to("cuda")
        )
        .argmax(dim=-1)
        .detach()
        .item()
    ...
    if os.path.exists(kmer_freqs_path):
        ...
        kmer_freqs = {
            s: {
                kmer_length: {"frequencies": None, "kmers": None}
        ...
        for s in range(10):
            filter_ds = val_dataset.filter(
                lambda x: x["species_label"] == s, num_proc=len(os.sched_getaffinity(0))
            )
            print(f"Computing kmer frequencies for species class {s}")
            for kmer_length in kmer_lengths:
                kmer_frequencies_gt, kmer_list = compute_kmer_frequencies(
                    seqs=filter_ds["sequence"], kmer_length=kmer_length
                )
    ...
    for s in range(10):
        ...
        for kmer_length in kmer_lengths:
            ...
            kmer_frequencies_generated, kmer_list = compute_kmer_frequencies(
                seqs=[i["sequence"] for i in test_data if i["species_label"] == s],
                kmer_length=kmer_length,
            )

            js_divergence = np.sum(
                scipy.spatial.distance.jensenshannon(
                    kmer_frequencies_gt, kmer_frequencies_generated
                )
            )
            kmer_results[kmer_length].append(js_divergence)
            mean_js_divergence += js_divergence
            print(f"\tJS divergence with k={kmer_length} : {js_divergence}")
        print(f"\tMean JS divergence : {mean_js_divergence / len(kmer_lengths):0.2f}")

    for k, v in kmer_results.items():
        weighted_kmer_js = (np.array(v) * np.array(list(val_weights.values()))).sum()
        print(
    ...
    disc_data = [{"sequence": i["sequence"], "species_label": 0} for i in test_data]
    for s in range(10):
        filter_val_ds = val_dataset.filter(
            lambda x: x["species_label"] == s, num_proc=len(os.sched_getaffinity(0))
        )
        indices = np.random.permutation(np.arange(len(filter_val_ds)))[
    ...
    disc_dataset_hf = datasets.Dataset.from_list(disc_data)
    disc_dataset_hf = disc_dataset_hf.train_test_split(test_size=0.1, seed=42)
    ...
    optimizer = torch.optim.AdamW(
        disc_model.parameters(), lr=lr, weight_decay=0, betas=(0.9, 0.999), eps=1e-8
    )
    ...
    for ep in tqdm(range(epochs), desc="Epochs"):
        # Train loop:
        disc_model.train()
        train_pbar = tqdm(disc_train_dl, desc="Train", leave=False)
        for batch in train_pbar:
            labels = batch["species_label"].to("cuda")
            logits = disc_model(batch["input_ids"].to("cuda")).logits
        ...
        for batch in disc_val_dl:
            disc_labels.append(batch["species_label"].numpy())
            disc_preds.append(
                disc_model(batch["input_ids"].to("cuda"))
                .logits[..., 1]
                .detach()
                .to("cpu")
                .numpy()
...
def main(config: omegaconf.DictConfig) -> None:
    ...
    if not os.path.exists(config.eval.generated_samples_path):
        ...
        for species in classes:
            ...
            print(
                "Guidance:",
                ", ".join(
                    [f"{k.capitalize()} - {v}" for k, v in config.guidance.items()]
                ),
            )
            samples = []
            for _ in tqdm(
    ...
    result_dict = run_eval_pipeline(
        samples_per_class,
        num_samples_per_class=config.sampling.num_sample_batches
