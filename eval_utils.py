...
def compute_generative_ppl(
    sentences, eval_model_name_or_path, gen_ppl_eval_batch_size=8, max_length=128
):
    ...
    if eval_model_tokenizer.pad_token is None:
        ...
        eval_model_tokenizer.pad_token_id = eval_model_tokenizer.eos_token_id
    ...
    samples = eval_model_tokenizer(sentences, **tokenizer_kwargs)
    attn_mask = samples["attention_mask"]
    samples = samples["input_ids"]
    attn_mask = attn_mask.to("cuda")
    samples = samples.to("cuda")
    num_batches = samples.shape[0] // gen_ppl_eval_batch_size
    for i in tqdm(range(num_batches), desc="Gen. PPL", leave=False):
