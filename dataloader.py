...
def get_dataset(
    ...
    if dataset_name == "text8":
        ...
        dataset = datasets.load_dataset(
            "amazon_polarity", cache_dir=cache_dir, streaming=streaming
        )
    elif dataset_name == "qm9":
        dataset = datasets.load_dataset(
            "yairschiff/qm9", cache_dir=cache_dir, streaming=streaming, split="train"
        )  # Dataset only has 'train' split
        if label_threshold is not None:
            pctiles = (
                label_threshold
                if isinstance(label_threshold, list)
                else [label_threshold]
            )
            pctile_values = np.percentile(dataset[label_col], q=pctiles)
    ...
    keep_cols = ["input_ids", "token_type_ids", "attention_mask"]
    if label_col is not None:
